...
class TestScene3 extends Phaser.Scene {
    constructor() {
        super('TestLevel3') // this scene has the key 'TestLevel3' when initializing it
    }
    ...
    startLevelTwo () {
        ...
        container.add(this.add.text(0, 70, 'This test aims to test the knowledge about Privacy by Default.', { fontFamily: 'Myriad Pro', fontSize: '25px', color: '#4D4D4D'}));
    ...
    chooseDifficulty (difficulty) {
        ...
        this.container.add(this.add.text(100, 120, 'What does Privacy by Default mean?', { fontFamily: 'Myriad Pro', fontSize: '30px', color: '#4D4D4D'} ));
        ...
        const alt1 = this.add.text(100, 180, '- Once a product or service has been released to the public, the strictest privacy settings should apply by default, without any manual input from the end user.', textStyle);
        ...
        alt1.on('pointerdown', function () {
            this.currentAnswer[0] = alt1.text.substr(2);
            this.container.destroy();
            this.nextQuestion();
        }, this);
        const alt2 = this.add.text(100, 260, '- Any action a company undertakes that involves processing personal data must be done with data protection and privacy in mind at every step.', textStyle);
        ...
        alt2.on('pointerdown', function () {
            this.currentAnswer[0] = alt2.text.substr(2);
            this.container.destroy();
            this.nextQuestion();
        }, this);
        const alt3 = this.add.text(100, 340, '- Any personal data provided by the user to enable a products optimal use should only be kept for the amount of time necessary to provide the product or service.', textStyle);
        ...
        alt3.on('pointerdown', function () {
            this.currentAnswer[0] = alt3.text.substr(2);
            this.container.destroy();
            this.nextQuestion();
        }, this);
    }
    ...
    nextQuestion () {
        ...
        this.container.add(this.add.text(100, 120, 'What does it means that privacy should be set by default?', { fontFamily: 'Myriad Pro', fontSize: '30px', color: '#4D4D4D'} ));
    ...
    updatePlayerStatistics () {
        this.player['statistics'][3]['attempts'] = this.attemptNumber.toString();
        this.player['statistics'][3]['time'] = this.currentTimer ? this.currentTimer.toString() : "None";
    }
    ...
    clearLevel () {
        ...
        const correctAnswer = [
            ...
            'Once a product or service has been released to the public, the strictest privacy settings should apply by default, without any manual input from the end user.'
        ]
        var correctCount = 0;
        ...
        if (correctCount == correctMax) {
            container.add(this.add.text(100, 380, 'Congratulations! You have completed all the levels for this demo.', { fontFamily: 'Myriad Pro', fontSize: '30px', color: '#4D4D4D'} ));
            ...
            this.player['achievements'].push(3);
